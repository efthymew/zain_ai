...
def get_model():
    ...
    # Apply a Conv2D layer to each image in the sequence
    model.add(TimeDistributed(Conv2D(filters=32, kernel_size=(3, 3), activation='relu', padding='same')))
    model.add(TimeDistributed(MaxPooling2D(pool_size=(2, 2))))  # Downsample the spatial dimensions
    model.add(TimeDistributed(Flatten()))  # Flatten the output before feeding into the LSTM
...
def get_model_from_file():
    return legacy_h5_format.load_model_from_hdf5("zain.h5", custom_objects={'mse': 'mse'})